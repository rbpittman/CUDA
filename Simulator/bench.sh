...
runBenchmark() {
    ...
    local CURR_SIZE=$NUM_PARTS_START
    while [ $CURR_SIZE -le $NUM_PARTS_END ]; do
	echo "${CURR_SIZE}" >> $FILENAME;
	echo "${CURR_SIZE}";
...
#	    local TIME=${RESULT:8:5};#Extract the exact time
	...
	let CURR_SIZE+=NUM_PARTS_STEP
    done
}

#ASSERT: N = 1,2,3, or 4
#SETUP FILE HEADER:
MESSAGE=" Benchmark for Particle Simulation";
if [ $SELECTION = 1 ]; then #PyCPU
    echo "PyCPU$MESSAGE" > $FILENAME;
    #runBenchmark "python gpuSimulator.py"
